...
window.onload = (function () {
    ...
    let btnStock = document.getElementById("id_btnActualizarProducto");
    ...
    $("#id_selectCategorias").change(function(){
        console.log("entra");
        cargaFamilias();
    });
});
...
let imagenModelo;
let CCODFAM; //Se usará más adelante
let CCODCAT;
function procesarPrecios() {
    ...
    if (solicitud_cargaPrecios.readyState == 4 && solicitud_cargaPrecios.status == 200) {
        ...
        imagenModelo = data[0].CIMAGEN;
        CCODFAM = data[0].CCODFAM;
        CCODCAT = data[0].NCODCAT;
        console.log("CCODCAT artículo: "+CCODCAT);
        console.log("CCODFAM artículo: "+CCODFAM);
    ...
    if (solicitud_cargaColores.readyState == 4 && solicitud_cargaColores.status == 200) {
        ...
        for (let n = 32; n <= 50; n += 2) {
            ...
            switch (n) {
                ...
                case 36:
                    tdBucle.innerHTML = "36<br><hr>S<br><hr>46";
                    break;

                case 38:
                    tdBucle.innerHTML = "38<br><hr>M<br><hr>48";
                    break;

                case 40:
                    tdBucle.innerHTML = "40<br><hr>L<br><hr>50";
                    break;

                case 42:
                    tdBucle.innerHTML = "42<br><hr>XL<br><hr>52";
                    break;

                case 44:
                    tdBucle.innerHTML = "44<br><hr>2XL<br><hr>54";
                    break;

                case 46:
                    tdBucle.innerHTML = "46<br><hr>3XL<br><hr>56";
                    break;

                case 48:
                    tdBucle.innerHTML = "48<br><hr>8<br><hr>58";
                    break;
                case 50:
                    tdBucle.innerHTML = "50<br><hr>10<br><hr>60";
                    break;
        ...
        for (let p = 0; p < jsonColores.length; p++) {
            ...
            switch (jsonColores[p].TALLA) {
                ...
                case '3XL':
                    ...
                    break;
                case '8':
                    inputTalla = document.getElementById(jsonColores[p].DESCRIPCION + "8");
                    inputTalla.setAttribute("value", jsonColores[p].NSTOCK);
                    break;

                case '10':
                    inputTalla = document.getElementById(jsonColores[p].DESCRIPCION + "10");
                    inputTalla.setAttribute("value", jsonColores[p].NSTOCK);
                    break;
...
function enviarPrecio() {
    ...
    value = document.getElementById("id_selectCategorias").value;
    textobusquedaenviado += "&NCODCAT=" + encodeURIComponent(value);
    value = document.getElementById("id_selectFamilias").value;
    textobusquedaenviado += "&CCODFAM=" + encodeURIComponent(value);
...
function llenarTabla(i) {
    ...
    input.setAttribute("class", "inpStock");
    input.setAttribute("width", "100%");
    input.setAttribute("id", jsonColores[i].DESCRIPCION + "8");
    tdTalla.appendChild(input);
    ...
    input.setAttribute("class", "inpStock");
    input.setAttribute("width", "100%");
    input.setAttribute("id", jsonColores[i].DESCRIPCION + "10");
    tdTalla.appendChild(input);
}
...
let solicitud_cargaCategorias;
function cargaCategorias() {
    solicitud_cargaCategorias = new XMLHttpRequest();
    solicitud_cargaCategorias.onreadystatechange = respuestaCategorias; //Función que se encarga de la respuesta que devuelve php
    solicitud_cargaCategorias.open('POST', 'php_peticiones/cargaCategorias.php', true); //Archivo php que se va a encargar de la petición
    solicitud_cargaCategorias.setRequestHeader("Content-Type", "application/x-www-form-urlencoded"); //Esto va siempre xd
    solicitud_cargaCategorias.send(); //Función que se encarga de mandar la información al php
}
...
function respuestaCategorias(){
    ...
    if (solicitud_cargaCategorias.readyState == 4 && solicitud_cargaCategorias.status == 200) {
        let dataCategorias = JSON.parse(solicitud_cargaCategorias.responseText);
        let select = document.getElementById('id_selectCategorias');

        for (let i = 0; i < dataCategorias.length; i++) {

            let value = dataCategorias[i].NCODCAT;
            let text = dataCategorias[i].CNOMCAT;
            if (dataCategorias[i].NCODCAT == CCODCAT){
                let option = document.createElement('option');
                option.value = value;
                option.text = text;
                option.selected = true;
                
                $("#id_selectCategorias").prepend(option); //Añade esta option al principio
...
function limpiarSelectCategorias(){

    var selectCategorias = document.getElementById('id_selectCategorias');
    while (selectCategorias.firstChild) {
        selectCategorias.removeChild(selectCategorias.lastChild);
    }
}


let solicitud_cargaFamilias;
function cargaFamilias() {
    solicitud_cargaFamilias = new XMLHttpRequest();
    solicitud_cargaFamilias.onreadystatechange = respuestaFamilias; //Función que se encarga de la respuesta que devuelve php
    solicitud_cargaFamilias.open('POST', 'php_peticiones/cargaFamilias.php', true); //Archivo php que se va a encargar de la petición
    solicitud_cargaFamilias.setRequestHeader("Content-Type", "application/x-www-form-urlencoded"); //Esto va siempre xd
    solicitud_cargaFamilias.send(mandaCategoria()); //Función que se encarga de mandar la información al php
}

function mandaCategoria(){
    let value = document.getElementById("id_selectCategorias").value;
    let textobusquedaenviado = "CCODCAT=" + encodeURIComponent(value)
    console.log("Categoria que voy a mandar: "+textobusquedaenviado);
    return textobusquedaenviado;
}

function respuestaFamilias(){

    limpiarSelectFamilias();

    if (solicitud_cargaFamilias.readyState == 4 && solicitud_cargaFamilias.status == 200) {
        let dataFamilias = JSON.parse(solicitud_cargaFamilias.responseText);
        let select = document.getElementById('id_selectFamilias');
        console.log(dataFamilias);
        for (let i = 0; i < dataFamilias.length; i++) {

            let value = dataFamilias[i].CCODFAM;
            let text = dataFamilias[i].CNOMFAM;

            if (dataFamilias[i].CCODFAM == CCODFAM){
                let option = document.createElement('option');
                option.value = value;
                option.text = text;
                option.selected = true;

                $("#id_selectFamilias").prepend(option); //Añade esta option al principio
...
function limpiarSelectFamilias(){

    let selectFamilias = document.getElementById('id_selectFamilias');
    while (selectFamilias.firstChild) {
        selectFamilias.removeChild(selectFamilias.lastChild);
    }
}
